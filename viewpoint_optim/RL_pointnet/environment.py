...
class ActiveAgent():
    def __init__(self, idx, n_points, seg_args=init_parser(),
        ...
        if self.mode == 'semantic':
            ...
            self.segmodel.load_state_dict(torch.load(
                os.path.join(self.args.model_dir, self.args.depth_fusion, 'epoch_15.pth')
            ))
    ...
    def reset(self, min_vis=True, up=3, verbose=False):
        ...
        return self._get_pcd_from_gt(gt), done
    ...
    def step(self, action):
        ...
        return self._get_pcd_from_gt(gt), reward, done
    ...
    def _get_gt(self):
        if self.mode == 'sim':
            ...
            if self.args.depth_fusion != 'no-depth':
                ...
                depth_trans = depth_trans.unsqueeze(0).cuda()
            else:
                depth_trans = None
    ...
    def _get_pcd_from_gt(self, gt, step=3):
        tgt_mask = np.ones(gt.shape).astype(np.int8)
        tgt_mask[gt != self.target_object] = -1

        d_im = np.load(os.path.join(self.scene_path, 'depth', '%d_depth_%d.npy' % tuple(self.coord)))
        points = []
        ...
        Z = d_im / self.scalingFactor
        x = (X - self.centerX) * Z / self.focalLength_x
        y = (Y - self.centerY) * Z / self.focalLength_y

        for v in range(0, d_im.shape[1], step):
            for u in range(0, d_im.shape[0], step):
                if Z[u][v] < 1e-4:
                    continue
                points.append([x[u][v], y[u][v], Z[u][v], tgt_mask[u][v]])
        ...
        points = points[np.random.randint(0, points.shape[0], size=self.n_points), :]
